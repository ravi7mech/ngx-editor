...
export function getMatch($position, opts) {
  ...
  }else if(text == opts.mentionTrigger){
    // openMentions as soon as Mention triggered
    type = "mention";
    ...
    state: {
      ...
      apply(tr, state) {
        ...
        if (match) {
          ...
          newState.openMentions = match.openMentions;
        }
    ...
    props: {
      ...
      /* handleDOMEvents: {
        ...
        focus: function(view, e) {
          var state = this.getState(view.state);
          if ((state && state.suggestions.length) || state.openMentions) {
            opts.getSuggestions(
              state.type,
              state.text,
    ...
    view() {
      return {
        update: view => {
          ...
          if (!state.text && !state.openMentions) {
            hideList();
            clearTimeout(showListTimeoutId);
            return;
          }
...
export default getMentionsPlugin;